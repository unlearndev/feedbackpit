...
`GET /dashboard` (`dashboard`) renders the `Dashboard` page, listing ideas with the most votes first, twelve to a page, with their author, votes, and subscriber data (serialized by `IdeaResource`). The `ideas` prop is a paginated resource collection, so the page reads the ideas from `ideas.data` and the previous/next links from `ideas.links` and `ideas.meta`.
